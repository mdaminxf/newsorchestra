...
/* safer href builder: returns a usable URL string or null */
...
// Enhanced QA Section Component
const  QASection = ({ qaSummary }) => {
  if (!qaSummary || !qaSummary.length) return null;

  // Only pick the three desired questions
  ...
  const filteredQAs = qaSummary.filter((qa) =>
    selectedQuestions.includes(qa.question)
  );

  return (
    <div className="bg-white rounded-xl shadow-md p-5 space-y-4">
      <h4 className="font-semibold text-gray-900 mb-2">Q&A Summary</h4>
...
export default function NewsOrchestraUI() {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      const result = await client.predict("/on_analyze_gradio", payload);
  ...
  const finalVerdictSummary = useMemo(() => {
    ...
    const claimSummaries = allReports.map((r, idx) => {
      ...
      const overall = r?.gemini_verdict?.overall || r?.summary || r?.qa_summary || "";
      return {
        idx,
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {allReports.map((r, i) => (
              <button
                ...
                className={`text-sm px-3 py-1 rounded ${VERDICT_COLORS[r.final_verdict || r.verdict || "Unclear"] || "bg-yellow-200 text-yellow-900"}`}
              >
    ...
    <div className="min-h-screen bg-gradient-to-br from-teal-50 to-indigo-50 py-8 px-4 text-gray-900">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-3">
            <div className="flex rounded-lg bg-white shadow p-1 border border-gray-200">
              <button onClick={() => setMode("gradio")} className={`px-4 py-1 rounded-lg ${mode === "gradio" ? "bg-teal-600 text-white" : "text-gray-600"}`}>Live</button>
              <button onClick={() => setMode("demo")} className={`px-4 py-1 rounded-lg ${mode === "demo" ? "bg-teal-600 text-white" : "text-gray-600"}`}>Demo</button>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
            <textarea
              value={
                mode !== "demo"
                  ? textOrUrl
                  : "SpiceJet Aircraft's Wheel Falls Off During Take-Off At Mumbai Airport, Full Emergency Declared"
              }
            ...
            <div className="md:col-span-4 flex flex-col gap-4">
              ...
              <div className="flex gap-3">
                <button
                  onClick={analyze}
                  disabled={loading}
                  className="flex-1 bg-gradient-to-r from-teal-600 to-teal-500 
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="md:col-span-2">
                ...
                <div className="space-y-4">
                  {finalVerdictSummary.map((cs) => (
                    <div key={cs.idx} className="p-4 border-l-4 border-teal-500 rounded bg-teal-50">
                      <div className="font-semibold text-gray-800">{cs.claim}</div>
                      <div className="mt-2 flex items-center gap-2">
                        <span className={`px-3 py-1 rounded-full text-sm font-medium ${BADGE_COLORS[cs.label] || "bg-gray-600 text-white"}`}>
                          {cs.label}
                        </span>
                        <span className="text-sm text-gray-600">
                          {confidenceTrustAggregates.avgConfidencePct != null ? `${confidenceTrustAggregates.avgConfidencePct}% confidence` : ""}
                        </span>
                      </div>
                      {cs.overall && (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* left: article */}
          <div className="bg-white rounded-xl shadow-md p-5 overflow-auto max-h-[60vh]">
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-xl shadow-md p-5 overflow-auto max-h-[36vh]">
  ...
  <QASection qaSummary={qaSummary} />
        ...
        <footer className="mt-12 text-center text-xs text-gray-500 select-none">
          <div className="max-w-3xl mx-auto p-4 rounded-lg bg-white border border-gray-200">
            ...
            <div className="mt-1">Fact-checking and analysis platform</div>
          </div>
        </footer>
      </div>
    </div>
